...
(CODIGO, TIPOS_CARGO_DESCARGO) = range(2)
...
class CargoDescargo:
    ...
    def __init__(self, padre = None):
        builder = gtk.Builder()
        builder.add_from_file('wTipo_De_Cargo_Descargo.glade')
    ...
    def mostrar_status(self):
        ...
        context_id = self.statusbar.get_context_id('Total de tipos de cargo y descargo registrados:')
        self.statusbar.push(context_id, buff)

    def crear_columnas(self):
        columnas = []
        columnas.append ([CODIGO, "Código", int])
        columnas.append ([TIPOS_CARGO_DESCARGO, "Tipos de cargo y descargo", str])
    ...
    def on_agregar_clicked(self,*args):
        dlg = DlgCargoDescargo(self.frm_padre, False)
        dlg.editando = False
    ...
    def on_quitar_clicked(self, *args):
        ...
        if yesno("¿Desea eliminar la operacion <b>%s</b>?\nEsta acción no se puede deshacer\n" % operacion, self.frm_padre) == gtk.RESPONSE_YES:
           Model().eliminar_cargo_descargo(codigo)
           model.remove(it)
...
class DlgCargoDescargo:
    ...
    def on_codigo_changed(self, *args):
        ...
        l = Model().buscar_id_cargo_descargo(codigo)
        if l:
    ...
    def on_guardar_clicked(self, *args):
        ...
        if lleno == 1 and not self.editando:
            Model().agregar_cargo_descargo(codigo, operacion)
            self.limpiar_todo()
            self.codigo.grab_focus()

        if lleno == 1 and self.editando:
            Model().modificar_cargo_descargo(codigo, operacion)
    ...
    def campos_llenos(self, codigo,nombre):
        ...
        if codigo == '':
            info("Debe colocar un codigo al tipo de cargo o descargo")
            return
