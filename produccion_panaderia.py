...
class ProduccionPanaderia:
    ...
    def mostrar_status(self):
        registros = Model().contar_registros('produccion_panaderia')
    ...
    def lista_ordenada_por_id(self,*args):
        self.cargar_lista(Model().producciones_panaderia_ordenadas_por_id())

    def cargar_lista(self, tupla):
        self.lista.clear()
        if tupla:
            for f in range(len(tupla)):
                self.lista.append([tupla[f][0],tupla[f][1], tupla[f][2], punto_coma(tupla[f][3])])
        else:
            info('No hay registros en produccion')

    def on_agregar_clicked(self,*args):
        dlg = DlgProduccionPanaderia()
    ...
    def on_quitar_clicked(self, *args):
        ...
        if yesno("¿Desea eliminar la produccion con código <b>%s</b>?\nEsta acción no se puede deshacer\n" % codigo, self.padre) == gtk.RESPONSE_YES:
           Model().eliminar_produccion_panaderia(codigo)
           model.remove(it)
    ...
    def on_tree_row_activated(self, *args):
        ...
        l = Model().buscar_id_produccion_panaderia(codigo)
        d = Model().buscar_detalles_id_produccion_panaderia(codigo)
...
class DlgProduccionPanaderia:
    def __init__(self, padre= None, editando = False):
        builder = gtk.Builder()
        builder.add_from_file("dlgProduccionPanes.glade")
        ...
        self.statusbar = builder.get_object('statusbar')
        self.agregar_receta_pan = builder.get_object('agregar_receta_pan')
        self.quitar_receta_pan = builder.get_object('quitar_receta_pan')
        self.limpiar_recetas_pan = builder.get_object('limpiar_recetas_pan')
        self.codigo = 0
        ...
        self.agregar_receta_pan.connect("clicked", self.on_agregar_receta_pan_clicked)
        self.quitar_receta_pan.connect("clicked", self.on_quitar_receta_pan_clicked)
        self.limpiar_recetas_pan.connect("clicked", self.on_limpiar_recetas_pan_clicked)
    ...
    def cargar_combo(self, *args):
        lista = Model().panaderos_ordenados_por_id()
    ...
    def on_agregar_receta_pan_clicked(self,*args):
        dlg = RecetasDePanesParaProduccion()
        response = dlg.dialogo.run()
        if dlg.resultado:
            self.cargar_receta(dlg.resultado)

    def on_quitar_receta_pan_clicked(self, *args):
        model, it = self.tree.get_selection().get_selected()
    ...
    def on_limpiar_recetas_pan_clicked(self,*args):
        self.lista.clear()
    ...
    def calcular_unidades_producidas(self, receta_id, harina):
        ...
        receta = list(Model().buscar_id_receta_pan_arroba(receta_id))
    ...
    def calcular_costo_arroba(self, receta_id, harina):
        harina_arroba = float(str(coma_punto(harina)))
        receta = list(Model().buscar_id_receta_pan_arroba(receta_id))
        nuevo_costo = float(receta[14]) * harina_arroba
    ...
    def on_aceptar_clicked(self, *args):
        ...
        if existe is None or existe == '':
            ...
            if lleno == 1 and filas >=1:
                produccion = Model().agregar_produccion_panaderia(fecha, estado, harina_arroba, harina_kg, harina_bultos, costos)
                ...
                if self.produccion_id != 0:
                    for i in self.lista:
                        ...
                        elaboraciones_insertadas = Model().agregar_produccion_detalles_panaderia(self.codigo, receta_id, harina, unidades, costo, empleado_id)
                else:
    ...
    def despachar(self, *args):
        ...
        for i in self.lista:
            ...
            empleado = Model().buscar_un_nombre_de_empleado(nombre_empleado)
            empleado_id = empleado[0]
            enc_receta = Model().buscar_id_receta_de_panaderia(receta_id)
            bruto = enc_receta[11]/4 * harina
            impuestos = enc_receta[12]/4 * harina
            total = enc_receta[14]/4 * harina
            ...
            for f in range(len(dtreceta)):
                ...
                if status == 1:
                    renglones = Model().agregar_despacho_detalles(despacho_id, cantidad, articulo_id, total_neto)

        if despacho_id != '' and renglones == filas:
           modificado = Model().produccion_panaderia_procesada(self.codigo)
           if modificado == 1:
               self.actualizar_estado(self.codigo)

    def actualizar_estado(self, produccion_id):
        encabezado = Model().buscar_id_produccion_panaderia(produccion_id)
...
class RecetasDePanesParaProduccion:
    ...
    def on_tree_row_activated(self, *args):
        (model, f) = self.tree.get_selection().get_selected()
        codigo = model.get_value(f,0)
        self.resultado = Model().buscar_id_receta_de_panaderia_para_produccion(codigo)
        if self.resultado:
            self.dialogo.destroy()


if __name__ == '__main__':
    ProduccionPanaderia().main()
