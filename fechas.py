#!/usr/bin/env python
# -*- coding: utf-8 -*-
# fechas -- Clase base para el manejo de empresas
...
# modified for spanish date by Fernando San Martín W., snmartin@galilea.cl 2004
...
_dayOfWeekName = ['Lunes', 'Martes', 'Miércoles', 'Jueves',
                  'Viernes', 'Sábado', 'Domingo']
_monthName = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio',
              'Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre']

def GetDateFromModel(fecha):
    if "/" in fecha:
        format = "%Y/%m/%d"
    else:
        format = "%Y-%m-%d"
    fecha = datetime.datetime(*time.strptime(fecha, format)[0:5])
    return fecha
...
class NormalDate:
    ...
    def formatES(self):
        ...
        return "%s/%s/%s" % (d[-2:], d[-4:-2], d[-6:-4])

    def formatESCentury(self):
        """return date as string in 4-digit year US format: MM/DD/YYYY"""
        d = self.__repr__()

        return "%s/%s/%s" % (d[-2:], d[-4:-2], d[-8:-4])
