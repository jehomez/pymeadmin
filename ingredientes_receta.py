...
class ArticulosDeProduccion:
    ...
    def __init__(self):
        builder = gtk.Builder()
        builder.add_from_file('wBuscar_Articulo_En_Produccion_Para_Receta.glade')
        builder.connect_signals(self)

        self.dialogo = builder.get_object('dialogo')
        self.dialogo.set_title('Lista de Articulos en Producción')
    ...
    def mostrar_status(self):
        ...
        buff = "Total de ingredientes registrados: %s" % registros
        context_id = self.statusbar.get_context_id('Total de ingredientes registrados: ')
    ...
    def on_filtro_changed(self, *args):
        if self.criterio.get_active() == 0:
            self.resultado = Model().buscar_id_articulo_produccion(self.filtro.get_text())

        elif self.criterio.get_active() == 1:
            self.resultado = Model().buscar_nombre_articulo_produccion(self.filtro.get_text())
    ...
    def on_tree_row_activated(self, *args):
        ...
        self.resultado = Model().buscar_id_articulo_produccion(codigo)
        self.dialogo.destroy()

if __name__ == '__main__':
    ArticulosDeProduccion().main()
