...
class ArticulosEnProduccion:
    ...
    def __init__(self):
        builder = gtk.Builder()
        builder.add_from_file('wLista_De_Articulos_En_Produccion.glade')
    ...
    def mostrar_status(self, filas):
        buff = "Total de artículos en producción registrados: %s" % filas
        context_id = self.statusbar.get_context_id('Total de artículos en producción registrados: ')
    ...
    def on_tree_row_activated(self, *args):
        ...
        self.resultado = Model().buscar_id_articulo(codigo)
        if self.resultado:
            self.dialogo.destroy()

if __name__ == '__main__':
    ArticulosEnProduccion().main()
