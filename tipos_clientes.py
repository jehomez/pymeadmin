...
class TiposClientes:
    ...
    def __init__(self, padre = None):
        builder = gtk.Builder()
        builder.add_from_file('wTipos_Clientes.glade')
    ...
    def mostrar_status(self):
        registros = Model().contar_registros('clientes_tipos')
        buff = "Total de tipos de clientes registrados: %s" % registros
        context_id = self.statusbar.get_context_id('Total de tipos de clientes registrados:')
    ...
    def lista_ordenada_por_id(self,*args):
        self.cargar_lista(Model().tipos_de_clientes_ordenados_por_id())
    ...
    def on_agregar_clicked(self,*args):
        dlg = DlgTipoCliente(self.frm_padre, False)
        dlg.editando = False
    ...
    def on_quitar_clicked(self, *args):
        ...
        if yesno("¿Desea eliminar el Proveedor de <b>%s</b>?\nEsta acción no se puede deshacer\n" % tipo, self.frm_padre) == gtk.RESPONSE_YES:
           Model().eliminar_tipo_cliente(codigo)
           model.remove(it)
    ...
    def on_filtro_changed(self, *args):
        if self.criterio.get_active() == 0:
            self.resultado = Model().buscar_id_tipo_cliente(self.filtro.get_text())
        else:
            self.resultado = Model().buscar_nombre_tipo_cliente(self.filtro.get_text())
    ...
    def on_imprimir_clicked(self,*args):
        t = treetohtml.TreeToHTML(self.tree,"Lista de Tipos de Clientes", self.col_data)
        t.show()
...
class DlgTipoCliente:

    def __init__(self, padre= None, editando = False):
        builder = gtk.Builder()
        builder.add_from_file("dlgTipo_Cliente.glade")
        builder.connect_signals(self)
    ...
    def on_guardar_clicked(self, *args):
        ...
        if lleno == 1 and not self.editando:
            Model().agregar_tipo_cliente(codigo, tipo)
            self.limpiar_todo()
            self.codigo.grab_focus()

        if lleno == 1 and self.editando:
            Model().modificar_tipo_cliente(codigo, tipo)
...
class dlgBuscarTipo:
    def __init__(self, padre= None):
        builder = gtk.Builder()
        builder.add_from_file("wBuscar_Tipo_Cliente.glade")
        builder.connect_signals(self)
        self.dlgBuscarTipo = builder.get_object("dlgBuscarTipoCliente")
        self.etiqueta = builder.get_object('lblValor')
        self.valor = builder.get_object("valor")
        self.criterio = builder.get_object("criterio")
        self.resultado =''
        self.dlgBuscarTipo.show()
