...
class RecetasDePasteleria:
    ...
    def __init__(self, padre = None):
        builder = gtk.Builder()
        builder.add_from_file('wRecetasPasteleria.glade')
    ...
    def mostrar_status(self):
        registros = Model().contar_registros('recetas_pasteleria')
        buff = "Total de recetas de pasteleria registradas: %s" % registros
        context_id = self.statusbar.get_context_id('Total de recetas de pasteleria registradas: ')
    ...
    def lista_ordenada_por_id(self,*args):
        self.cargar_lista(Model().recetas_de_pasteleria_ordenadas_por_id())
    ...
    def on_quitar_clicked(self, *args):
        ...
        if yesno("¿Desea eliminar la receta <b>%s</b>?\nEsta acción no se puede deshacer\n" % receta, self.padre) == gtk.RESPONSE_YES:
           Model().eliminar_receta_de_pasteleria(codigo)
           model.remove(it)
    ...
    def on_filtro_changed(self, *args):
        if self.criterio.get_active() == 0:
            self.resultado = Model().buscar_id_receta_pasteleria(self.filtro.get_text())

        elif self.criterio.get_active() == 1:
            self.resultado = Model().buscar_nombre_de_receta_de_pasteleria(self.filtro.get_text())
    ...
    def on_tree_row_activated(self, *args):
        ...
        l = Model().buscar_id_receta_de_pasteleria(codigo)
        d = Model().buscar_detalles_id_receta_pasteleria(codigo)
...
class DlgReceta:
    def __init__(self, padre= None, editando = False):
        builder = gtk.Builder()
        builder.add_from_file("dlgRecetaPasteleria.glade")

        self.dialogo = builder.get_object("dialogo")
        self.receta_pasteleria_id = builder.get_object("receta_pasteleria_id")
    ...
    def on_codigo_changed(self, *args):
        ...
        if len(codigo) >= 11:
            l = Model().buscar_id_receta_pasteleria(codigo)
    ...
    def limpiar_todo(self, *args):
        self.receta_pasteleria_id.set_text('')
    ...
    def campos_llenos(self, receta_pasteleria_id, nombre, articulo_id):
        ok = 0

        if receta_pasteleria_id == '':
        ...
        if receta_pasteleria_id != '' and nombre != '' and articulo_id != '':
            ok = 1
        else:
            ok = 0
    ...
    def on_aceptar_clicked(self, *args):
        receta_pasteleria_id = self.receta_pasteleria_id.get_text()
        ...
        lleno = self.campos_llenos(receta_pasteleria_id, nombre, articulo_id)
        filas = len(self.lista)
        existe = Model().buscar_id_receta_de_pasteleria(receta_pasteleria_id)
        if not existe:
            if lleno == 1 and filas >=1:
                insertado = Model().agregar_receta_de_pasteleria(receta_pasteleria_id, nombre, articulo_id, mezcla, mano_obra, total)
                if insertado == 1:
                    for i in self.lista:
                        cantidad = coma_punto(i[0])
                        articulo_id = i[1]
                        total_neto = coma_punto(i[7])
                        ingredientes_insertados = Model().agregar_recetas_pasteleria_detalles(receta_pasteleria_id, cantidad, articulo_id, total_neto)
                else:
            ...
            if insertado == 1 and ingredientes_insertados >= 1:
                Model().establecer_costo(articulo_id, total)
                self.limpiar_todo()
        else:
            modificado = Model().modificar_receta_de_pasteleria(nombre, articulo_id, mezcla, mano_obra, total, receta_pasteleria_id)
...
class Ingredientes:
    ...
    def mostrar_status(self, filas):
        registros = Model().contar_ingredientes()
    ...
    def on_tree_row_activated(self, *args):
        (model, f) = self.tree.get_selection().get_selected()
        codigo = model.get_value(f,0)
        self.resultado = Model().buscar_ingrediente(codigo)
        print self.resultado
        if self.resultado:
            self.dialogo.destroy()

if __name__ == '__main__':
    RecetasDePasteleria().main()
