...
class Operaciones:
    ...
    def __init__(self, padre = None):
        builder = gtk.Builder()
        builder.add_from_file('wOperaciones.glade')
        builder.connect_signals(self)

        self.ventana = builder.get_object('ventana')
        self.treeOperaciones = builder.get_object('tree')
    ...
    def mostrar_status(self):
        registros = Model().contar_registros('tipo_operacion')
        buff = "Total de tipos de operaciones registradas: %s" % registros
        context_id = self.statusbar.get_context_id('Total de tipos de operaciones registradas:')
        self.statusbar.push(context_id, buff)

    def crear_columnas(self):
        columnas = []
        columnas.append ([CODIGO, "Código", int])
        columnas.append ([OPERACIONES, "Operaciones", str])
    ...
    def on_agregar_clicked(self,*args):
        dlg = DlgOperacion(self.frm_padre, False)
        dlg.editando = False
    ...
    def on_quitar_clicked(self, *args):
        ...
        operacion = model.get_value(it,1)

        if yesno("¿Desea eliminar la operacion <b>%s</b>?\nEsta acción no se puede deshacer\n" % operacion, self.frm_padre) == gtk.RESPONSE_YES:
           Model().eliminar_operacion(codigo)
           model.remove(it)
    ...
    def on_filtro_changed(self, *args):
        ...
        if self.criterio.get_active() == 0:
            ...
            self.resultado = Model().buscar_id_operacion(self.filtro.get_text())

        elif self.criterio.get_active() == 1:

            self.resultado = Model().buscar_nombre_operacion(self.filtro.get_text())
    ...
    def on_imprimir_clicked(self,*args):
        t = treetohtml.TreeToHTML(self.tree,"Lista de Tipos de Operaciones", self.col_data)
        t.show()
    ...
    def on_tree_row_activated(self, *args):
        ...
        dlg.operacion.set_text(model.get_value(f,1))
        dlg.operacion.grab_focus()
        response = dlg.dlgOperacion.run()
        if response == gtk.RESPONSE_OK:
           self.lista.clear()
           self.lista_ordenada_por_id()
...
class DlgOperacion:
    ...
    def __init__(self, padre= None, editando = False):
        builder = gtk.Builder()
        builder.add_from_file("dlgOperacion.glade")
        builder.connect_signals(self)
    ...
    def on_codigo_changed(self, *args):
        ...
        if l:
            self.cargar_operacion(l)
        else:
            self.limpiar()

    def on_guardar_clicked(self, *args):
        codigo = self.codigo.get_text()
        operacion = self.operacion.get_text()
        lleno = self.campos_llenos(codigo, operacion)

        if lleno == 1 and not self.editando:
            Model().agregar_operacion(codigo, operacion)
            self.limpiar_todo()
            self.codigo.grab_focus()

        if lleno == 1 and self.editando:
            Model().modificar_operacion(codigo, operacion)
